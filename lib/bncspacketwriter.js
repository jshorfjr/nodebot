...
BNCSPacketWriter.prototype.sendAuthCheck = function (conn, clientToken, exeVersion, exeHash, keyHash, exeInfo, username) {
	var pkt = new BNCSPacket([0x51]);
	pkt.writeUint32(clientToken);
	pkt.writeUint32(exeVersion);
	pkt.writeUint32(exeHash);
	pkt.writeUint32(1);
	pkt.writeUint32(0);
	pkt.writeCustom(keyHash);
	pkt.writeString(exeInfo);
	pkt.writeString(username);
	conn.write(pkt.constructPacket());
	console.log("[bncs c>s] 0x51");
